...
if (isset($_POST['step']) && $_POST['step'] > 1 && !isset($_SESSION['coord_reset_email'])) {
    $_POST['step'] = 1;
}
    ...
    try {
        ...
        if ($_POST['step'] == 1) {
            ...
            if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                // STRICT CHECK: Must be a coordinator account
                $stmt = $pdo->prepare("SELECT id, full_name, is_active FROM users WHERE email = ? AND role = 'coordinator'");
                $stmt->execute([$email]);
                $user = $stmt->fetch(PDO::FETCH_ASSOC);
                
                if ($user) {
                    if (!$user['is_active']) {
                        $error = "This coordinator account is disabled. Contact system administration.";
                        $step = 1;
                    } else {
                        // Generate 6-digit OTP
                        $otp = rand(100000, 999999);
                        $_SESSION['coord_reset_email'] = $email;
                        $_SESSION['coord_reset_otp'] = $otp;
                        $_SESSION['coord_reset_name'] = $user['full_name'];
                        
                        // Send OTP via Brevo Mailer (Purple Theme)
                        $html_body = "
                            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 25px; border: 1px solid #E2E8F0; border-radius: 16px; background: #FFFFFF;'>
                                <div style='text-align: center; margin-bottom: 20px;'>
                                    <h2 style='color: #7C3AED; margin: 0;'>Coordinator Password Reset</h2>
                                    <p style='color: #64748B; font-size: 14px; margin-top: 5px;'>NIELIT Exam Logistics Portal</p>
                                </div>
                                <p style='color: #0F172A; font-size: 15px;'>Dear <strong>{$user['full_name']}</strong>,</p>
                                <p style='color: #475569; font-size: 15px; line-height: 1.6;'>A password reset was requested for your Coordinator account. Here is your secure One-Time Password (OTP):</p>
                                
                                <div style='background: #F5F3FF; padding: 20px; border-radius: 12px; border: 1px dashed #7C3AED; margin: 25px 0; text-align: center;'>
                                    <h1 style='margin: 0; color: #7C3AED; font-size: 36px; letter-spacing: 5px;'>{$otp}</h1>
                                </div>
                        ...
                        if (sendNielitEmail($email, $user['full_name'], "NIELIT Coordinator: Password Reset OTP", $html_body)) {
                    ...
                    $error = "If this email is registered as a Coordinator, an OTP has been sent.";
                    $step = 1;
                }
            }
        }
        ...
        elseif ($_POST['step'] == 2) {
            ...
            if ($entered_otp == $_SESSION['coord_reset_otp']) {
        ...
        elseif ($_POST['step'] == 3) {
            ...
            if (strlen($new_password) < 8) {
                $error = "Coordinator passwords must be at least 8 characters long for security.";
                $step = 3;
                ...
                $email_to_update = $_SESSION['coord_reset_email'];
                
                $updateStmt = $pdo->prepare("UPDATE users SET password_hash = ?, password = ? WHERE email = ? AND role = 'coordinator'");
                $updateStmt->execute([$hashed_password, $new_password, $email_to_update]);
                
                // Clear reset session data
                unset($_SESSION['coord_reset_email']);
                unset($_SESSION['coord_reset_otp']);
                unset($_SESSION['coord_reset_name']);
                
                $success = "Coordinator password successfully updated! You may now securely log in.";
        ...
        error_log("Coordinator Password Reset DB error: " . $e->getMessage());
        $step = 1;
    }
}
?>
...
<head>
    ...
    <title>Coordinator Password Reset - NIELIT</title>
    ...
    <style>
        ...
        /* Modern Purple Abstract Background */
        ...
        .login-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px); border: 1px solid white; border-radius: 24px; padding: 40px; width: 100%; max-width: 420px; box-shadow: 0 25px 50px -12px rgba(124, 58, 237, 0.15); animation: fadeUp 0.6s ease-out; position: relative; z-index: 1;}
        @keyframes fadeUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }

        .logo-wrap { text-align: center; margin-bottom: 25px; }
        .icon-box { width: 64px; height: 64px; background: var(--primary); color: white; border-radius: 18px; display: inline-flex; align-items: center; justify-content: center; font-size: 28px; margin-bottom: 15px; box-shadow: 0 10px 25px rgba(124, 58, 237, 0.3); }
        ...
        .form-control:focus { border-color: var(--primary); background: white; box-shadow: 0 0 0 4px #EDE9FE; }
        ...
        .btn-submit { width: 100%; padding: 15px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: 700; font-size: 15px; font-family: inherit; cursor: pointer; transition: 0.3s; margin-top: 5px; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 15px rgba(124, 58, 237, 0.25); }
        .btn-submit:hover { background: var(--primary-hover); transform: translateY(-2px); box-shadow: 0 8px 20px rgba(124, 58, 237, 0.35); }
...
<body>
    ...
    <a href="coordinator-login.php" class="back-link"><i class="fas fa-arrow-left"></i> Back to Login</a>
    ...
    <div class="login-card">
        <div class="logo-wrap">
            ...
            <p>
                <?php 
                    if ($step == 1) echo "Enter your registered coordinator email address.";
                    elseif ($step == 2) echo "We sent a secure 6-digit OTP to <br><b style='color: var(--text-dark); margin-top: 5px; display: block;'>" . htmlspecialchars($_SESSION['coord_reset_email']) . "</b>";
                    elseif ($step == 3) echo "Create a strong new coordinator password.";
                ?>
            </p>
        </div>
        ...
        <form method="POST" autocomplete="off">
            ...
            <div class="form-group">
                ...
                <div class="input-wrap">
                    ...
                    <input type="email" name="email" class="form-control" placeholder="coordinator@institute.edu" value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>" required autofocus>
                </div>
            </div>
        ...
        <div class="success-state">
            ...
            <a href="coordinator-login.php" class="btn-submit" style="text-decoration: none;">
