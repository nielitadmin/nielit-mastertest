...
try {
    ...
    if (!$exam) {
        header("Location: manage-exams.php?msg=Exam not found");
        exit();
    }
    ...
    $centers = $pdo->query("SELECT id, center_code, center_name, city, capacity, is_active FROM exam_centers ORDER BY center_name")->fetchAll(PDO::FETCH_ASSOC);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $conductor_name = trim($_POST['conductor_name']); // Exam Conductor Name
        $is_active = isset($_POST['is_active']) ? 'true' : 'false';
        $is_practice = isset($_POST['is_practice']) ? 'true' : 'false';
        ...
        if (empty($category_id)) {
        ...
        } elseif ($is_practice === 'false' && (empty($center_id) || empty($exam_date) || empty($start_time) || empty($end_time) || empty($total_seats))) {
            $error = "Please fill in all required fields for a formal exam.";
        } elseif ($is_practice === 'false' && $end_time <= $start_time) {
            ...
            $update = $pdo->prepare("
                ...
                SET category_id = ?, center_id = ?, exam_date = ?, start_time = ?, end_time = ?, total_seats = ?, is_active = ?, is_practice = ?, exam_conductor = ?
                WHERE id = ?
            ");
            $update->execute([$category_id, $center_id, $exam_date, $start_time, $end_time, $total_seats, $is_active, $is_practice, $conductor_name, $exam_id]);
            
            $pdo->commit();
            
            header("Location: manage-exams.php?msg=Exam Session Updated Successfully");
            exit();
        }
    }
...
$catData = [];
foreach($categories as $c) { $catData[$c['id']] = $c['duration_minutes']; }
$cenData = [];
...
<body>
    ...
    <div class="container">
        ...
        <div class="page-header">
            <h1>Update Exam Session</h1>
            <p>Modify the configuration for existing exam code: <strong><?php echo htmlspecialchars($exam['exam_code']); ?></strong></p>
        </div>
        ...
        <form method="POST" class="form-card" autocomplete="off">
            ...
            <div class="practice-banner">
                <div>
                    ...
                    <p>Enable unlimited seats and flexible 24/7 scheduling.</p>
                </div>
                <label class="switch">
                    <input type="checkbox" name="is_practice" id="practiceToggle" value="true" onchange="togglePracticeMode()" <?php echo ($exam['is_practice']) ? 'checked' : ''; ?>>
            ...
            <div class="form-grid">
                ...
                <div class="form-group full-width">
                    <label>Assigned Exam Code (Read-Only)</label>
                    <div class="input-wrap">
                        <input type="text" value="<?php echo htmlspecialchars($exam['exam_code']); ?>" class="form-control" readonly>
                ...
                <div class="form-group full-width">
                    ...
                    <div class="input-wrap">
                        <input type="text" name="conductor_name" class="form-control" placeholder="E.g. Mr. Rajesh Kumar / NIELIT BBSR Team" required value="<?php echo htmlspecialchars($exam['exam_conductor'] ?? ''); ?>">
            ...
            <div id="formal-settings">
                    ...
                    <div class="input-wrap">
                        <select name="center_id" id="center_id" class="form-control req-formal" required onchange="updateCapacityHint()">
                            ...
                            <?php foreach ($centers as $cen): ?>
                                <option value="<?php echo $cen['id']; ?>" <?php echo ($exam['center_id'] == $cen['id']) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($cen['center_name']) . ' - ' . htmlspecialchars($cen['city']); ?>
                                    <?php echo !$cen['is_active'] ? ' (Inactive)' : ''; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <div class="input-wrap">
                            <input type="date" name="exam_date" class="form-control req-formal" value="<?php echo htmlspecialchars($exam['exam_date'] ?? ''); ?>" required>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrap">
                            <input type="time" name="start_time" id="start_time" class="form-control req-formal" required value="<?php echo htmlspecialchars($exam['start_time'] ?? ''); ?>">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrap">
                            <input type="time" name="end_time" id="end_time" class="form-control req-formal" required value="<?php echo htmlspecialchars($exam['end_time'] ?? ''); ?>">
                ...
                <div class="form-group full-width">
                    ...
                    <div class="input-wrap">
                        <input type="number" name="total_seats" id="total_seats" class="form-control req-formal" min="1" placeholder="Enter maximum seat limit..." required value="<?php echo htmlspecialchars($exam['total_seats'] ?? ''); ?>">
            ...
            <div class="form-group full-width">
                <div style="display: flex; align-items: center; justify-content: space-between; background: white; border: 2px solid var(--border); padding: 18px 24px; border-radius: var(--radius-md);">
                    ...
                    <label class="switch">
                        <input type="checkbox" name="is_active" value="1" <?php echo ($exam['is_active']) ? 'checked' : ''; ?>>
    ...
    <script>
        ...
        const savedCategoryId = "<?php echo $exam['category_id']; ?>";
        ...
        function updateModules() {
            ...
            if(parentVal && categoryData[parentVal]) {
                categoryData[parentVal].forEach(mod => {
                    ...
                    if(mod.id == savedCategoryId) opt.selected = true;
        ...
        // Initialize state on load to pre-fill the cascading dropdowns
        window.onload = function() {
            togglePracticeMode();
            
            if(savedCategoryId) {
                for(const [catName, modules] of Object.entries(categoryData)) {
                    if(modules.some(m => m.id == savedCategoryId)) {
