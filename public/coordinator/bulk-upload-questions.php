...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['csv_file'])) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            $handle = fopen($file['tmp_tmp'] ?? $file['tmp_name'], "r");
            if ($handle !== FALSE) {
                // Skip header row
                fgetcsv($handle);
                ...
                while (($data = fgetcsv($handle, 2000, ",")) !== FALSE) {
                    ...
                    if (empty($qText)) {
                        throw new Exception("Row $rowNum: Question text cannot be empty.");
                    }
                    ...
                    // Handle Options
                    if ($qType === 'mcq') {
                        $optionsList = array_filter([$opt1, $opt2, $opt3, $opt4]);
                        if (count($optionsList) < 2) {
                            throw new Exception("Row $rowNum: MCQ questions must have at least 2 options.");
                        }
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* --- MAIN CONTAINER --- */
        .container { max-width: 800px; margin: 40px auto; padding: 0 20px; position: relative; z-index: 10; animation: fadeUp 0.5s ease-out;}
        ...
        .alert { padding: 16px 20px; border-radius: var(--radius-md); font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 10px; margin-bottom: 20px; animation: slideIn 0.4s ease; line-height: 1.5;}
        ...
        .info-icon { font-size: 24px; color: var(--primary); }
        ...
        .btn-download:hover { background: var(--primary-bg); border-color: var(--primary-light); }
        ...
        .upload-zone i { font-size: 40px; color: var(--primary-light); margin-bottom: 15px; transition: 0.3s; }
        .upload-zone:hover i { transform: translateY(-5px); color: var(--primary); }
...
<body>
    ...
    <div class="container">
        ...
        <div class="page-header">
            <h1><i class="fas fa-file-csv" style="color: var(--primary);"></i> Bulk Upload Questions</h1>
            <p>Upload hundreds of questions instantly using a standard CSV format.</p>
        </div>
        ...
        <?php if ($success): ?>
            <div class="alert alert-success">
                ...
                <a href="manage-questions.php" style="margin-left:auto; color:var(--success); text-decoration:underline;">View Bank</a>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" enctype="multipart/form-data" class="card">
            ...
            <div class="action-row">
                <a href="manage-questions.php" class="btn btn-cancel">Cancel</a>
