...
<head>
    ...
    <title>View Exam Roster - NIELIT Coordinator</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .ambient-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: -1; pointer-events: none; background: linear-gradient(180deg, #F8FAFC 0%, #E2E8F0 100%); }
        
        .navbar-wrapper { position: sticky; top: 0; z-index: 1000; background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(16px); border-bottom: 1px solid var(--border); box-shadow: 0 4px 30px -10px rgba(0,0,0,0.05); }
        ...
        .btn-back { display: inline-flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--border); padding: 8px 16px; border-radius: 10px; color: var(--text-dark); text-decoration: none; font-weight: 600; font-size: 13px; transition: 0.3s; box-shadow: var(--shadow-sm);}
        ...
        .container { max-width: 1400px; margin: 30px auto; padding: 0 40px; animation: fadeUp 0.5s ease-out;}
        ...
        .badge { padding: 6px 14px; border-radius: 50px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; display: inline-flex; align-items: center; gap: 6px; margin-bottom: 20px;}
        .status-upcoming { background: var(--warning-bg); color: var(--warning); border: 1px solid #FDE68A;}
        .status-ongoing { background: var(--success-bg); color: var(--success); border: 1px solid #A7F3D0; animation: pulse 2s infinite;}
        .status-completed { background: var(--neutral-bg); color: var(--neutral); border: 1px solid #E2E8F0;}
        .status-practice { background: var(--practice-bg); color: var(--practice); border: 1px solid #DDD6FE;}
        .status-inactive { background: var(--danger-bg); color: var(--danger); border: 1px solid #FECACA;}
        @keyframes pulse { 0% { box-shadow: 0 0 0 0 rgba(16, 185, 129, 0.4); } 70% { box-shadow: 0 0 0 6px rgba(16, 185, 129, 0); } 100% { box-shadow: 0 0 0 0 rgba(16, 185, 129, 0); } }

        .exam-header h1 { font-size: 24px; font-weight: 800; color: var(--text-dark); margin-bottom: 5px; }
        .exam-header p { font-size: 14px; color: var(--primary); font-weight: 700; margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px dashed var(--border); }

        .detail-row { display: flex; gap: 15px; margin-bottom: 18px; }
        .detail-icon { width: 38px; height: 38px; background: var(--neutral-bg); color: var(--neutral); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 14px; flex-shrink: 0; }
        .detail-text h4 { font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
        .detail-text p { font-size: 14px; font-weight: 600; color: var(--text-dark); line-height: 1.4;}
        
        /* Themed Icons */
        .ic-loc { background: var(--primary-bg); color: var(--primary); }
        .ic-cal { background: var(--warning-bg); color: var(--warning); }
        .ic-prac { background: var(--practice-bg); color: var(--practice); }

        /* Mini Stats */
        .mini-stats { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 30px; border-top: 1px dashed var(--border); padding-top: 25px;}
        ...
        .m-stat p { font-size: 11px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; margin-top: 4px; }

        /* Right Panel - Roster Table */
        .table-toolbar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;}
        .table-toolbar h2 { font-size: 18px; font-weight: 800; color: var(--text-dark); display: flex; align-items: center; gap: 10px;}
        ...
        .search-box input { width: 100%; padding: 12px 16px 12px 42px; border-radius: 50px; border: 1px solid var(--border); background: var(--bg-body); font-family: inherit; font-size: 13px; font-weight: 500; outline: none; transition: 0.3s; }
        .search-box input:focus { border-color: var(--primary); background: white; box-shadow: 0 0 0 3px var(--primary-bg);}

        .table-responsive { overflow-x: auto; background: white; border-radius: var(--radius-md); border: 1px solid var(--border); }
        table { width: 100%; border-collapse: collapse; min-width: 800px; }
        th { background: #F8FAFC; color: var(--text-muted); font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; padding: 16px 20px; text-align: left; border-bottom: 2px solid var(--border); }
        ...
        .cand-info strong { color: var(--text-dark); font-weight: 800; }
        .cand-info span { font-size: 12px; color: var(--text-muted); font-weight: 600;}

        .status-pill { padding: 6px 12px; border-radius: 50px; font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; }
        ...
        .score-box { font-weight: 800; font-size: 16px; display: inline-block; padding: 4px 10px; border-radius: 8px;}
...
<body>
    ...
    <div class="container">
        ...
        <?php if ($error): ?>
            <div style="background: var(--danger-bg); color: var(--danger); padding: 15px 20px; border-radius: 12px; margin-bottom: 25px; font-weight: 600; border: 1px solid #FECACA; display: flex; align-items: center; gap: 10px;">
        ...
        <div class="dashboard-grid">
            ...
            <div class="bento-card">
                <span class="badge status-<?php echo $status; ?>">
                    <?php 
                        ...
                        elseif($status == 'ongoing') echo '<i class="fas fa-broadcast-tower"></i> Live Now';
                        else echo '<i class="fas fa-calendar-alt"></i> ' . ucfirst($status); 
                    ?>
                </span>
                
                <div class="exam-header">
                    <h1><?php echo htmlspecialchars($exam['exam_code']); ?></h1>
                    <p><?php echo htmlspecialchars($exam['category_code'] . ' - ' . $exam['category_name']); ?></p>
                </div>

                <?php if ($exam['is_practice']): ?>
                    <div class="detail-row">
                        <div class="detail-icon ic-prac"><i class="fas fa-globe"></i></div>
                    ...
                    <div class="detail-row">
                        <div class="detail-icon ic-prac"><i class="fas fa-clock"></i></div>
                        <div class="detail-text">
                            <h4>Duration</h4>
                ...
                <?php else: ?>
                    <div class="detail-row">
                        <div class="detail-icon ic-loc"><i class="fas fa-building"></i></div>
                        <div class="detail-text">
                            <h4>Exam Center</h4>
                    ...
                    <div class="detail-row">
                        <div class="detail-icon ic-cal"><i class="far fa-calendar-alt"></i></div>
                        <div class="detail-text">
                            <h4>Schedule</h4>
                ...
                <div class="mini-stats">
                    <div class="m-stat">
                        <h3><?php echo $stats['total_registered']; ?><?php echo $exam['is_practice'] ? '' : '<span style="color:var(--text-muted); font-size:16px;">/' . $exam['total_seats'].'</span>'; ?></h3>
                        <p>Registered</p>
                    </div>
                    <div class="m-stat">
            ...
            <div class="bento-card" style="padding: 25px;">
                <div class="table-toolbar">
                    <h2><i class="fas fa-users" style="color: var(--primary);"></i> Candidate Roster</h2>
                            ...
                            <?php if (empty($roster)): ?>
                                <tr>
                                    <td colspan="4" style="text-align: center; padding: 60px 20px; color: var(--text-muted);">
                                        <i class="fas fa-users-slash" style="font-size: 32px; margin-bottom: 15px; display: block; color: var(--border);"></i>
                                        <span style="font-weight: 600;">No candidates have registered for this session yet.</span>
                                    </td>
                                </tr>
                            <?php else: ?>
